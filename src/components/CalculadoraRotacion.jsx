...
export default function CalculadoraRotacion() {
  ...
  const impact = calculateImpact()
  const inputFieldClass = 'w-full px-4 py-2 border border-brand-navy/15 bg-white rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-gold transition-shadow'

  if (step === 'inputs') {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-2xl mx-auto p-6">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-brand-navy mb-4">💰 Calculadora de Costo por Rotación</h2>
          <p className="text-brand-ink-soft mb-6">
            Descubre cuánto te cuesta realmente la rotación de personal en tu empresa.
          </p>
        </div>

        <motion.div className="bg-white p-6 rounded-lg ring-1 ring-brand-navy/5 shadow-card space-y-6">
          <div>
          ...
          <div>
            ...
            <input
              ...
              className={inputFieldClass}
            />
            <p className="text-xs text-brand-ink-soft mt-1">Referencia: 20% es normal, 30%+ es alto</p>
          </div>
          ...
          <motion.button
            ...
            className="w-full px-8 py-3 bg-brand-gold text-white rounded-lg font-semibold shadow-card hover:bg-brand-gold-dark hover:shadow-card-hover transition-colors duration-300"
          >
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-2xl mx-auto p-6">
      <div className="bg-red-50 p-8 rounded-lg mb-8 border-l-4 border-red-600">
        <p className="text-brand-ink-soft text-sm mb-2">Costo anual por rotación:</p>
        ...
        <p className="text-sm text-brand-ink-soft">Por año en tu empresa</p>
      </div>

      <div className="grid grid-cols-2 gap-4 mb-8">
        <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white p-4 rounded-lg ring-1 ring-brand-navy/5 shadow-card">
          <p className="text-xs text-brand-ink-soft">Empleados rotados/año</p>
          <p className="text-2xl font-bold text-brand-navy">{impact.colaboradoresRotados}</p>
        </motion.div>
        <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="bg-white p-4 rounded-lg ring-1 ring-brand-navy/5 shadow-card">
          <p className="text-xs text-brand-ink-soft">Por empleado</p>
          <p className="text-2xl font-bold text-brand-navy">S/ {parseFloat(impact.costoAnualPorEmpleado).toLocaleString('es-ES', { minimumFractionDigits: 2 })}</p>
        </motion.div>
      </div>

      <div className="bg-white p-6 rounded-lg ring-1 ring-brand-navy/5 shadow-card mb-8">
        <h3 className="font-semibold text-brand-navy mb-4">📊 Desglose de costos:</h3>
        <div className="space-y-3 text-sm">
          <div className="flex justify-between">
            <span className="text-brand-ink">Costos directos (reclutamiento + capacitación)</span>
            <span className="font-semibold text-brand-navy">S/ {parseFloat(impact.costosDirectos).toLocaleString('es-ES', { minimumFractionDigits: 2 })}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-brand-ink">Pérdida de productividad</span>
            <span className="font-semibold text-brand-navy">S/ {parseFloat(impact.costosPerdidaProductividad).toLocaleString('es-ES', { minimumFractionDigits: 2 })}</span>
          </div>
          <div className="border-t border-brand-navy/10 pt-3 flex justify-between">
            <span className="text-brand-navy font-semibold">Total anual</span>
      ...
      <form onSubmit={handleSubmit} className="bg-brand-cream p-6 rounded-lg">
        <h3 className="font-semibold text-brand-navy mb-4">📩 Recibe un plan de acción:</h3>
